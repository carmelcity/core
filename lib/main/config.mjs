...
const loadSSL = async () => {
    ...
    const cert = fs.readFileSync(sslCertFile, 'utf8');
 ...
 * @returns
 */
const relayConfig = ({ privateKey, peerId, ssl, }) => {
 ...
 * @returns
 */
const sentinalConfig = ({ privateKey }) => {
    return {
        p2p: {
            privateKey,
            start: true,
...
export const makeConfig = async () => {
    ...
    const type = nodeType();
    const { peerId, privateKey } = await createNodeId();
